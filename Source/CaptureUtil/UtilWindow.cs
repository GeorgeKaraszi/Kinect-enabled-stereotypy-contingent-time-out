...
namespace WesternMichgian.SeniorDesign.KinectProject.CaptureUtil
{
    public partial class UtilWindow : Form
    {
        ...
        public void UpdateTracking(int bodyId, bool trackingStatus)
        {
            ...
            else if (trackingStatus == false)
            {
                ...
                RemoveRecording(targetPanel);

                //Verify panels are in correct order after a deletion
                RearrangePanels();
            }
        ...
        /// </param>
        public void UpdateData(int bodyid, Tuple<int, double> frame)
        {
        ...
        private void UpdateChartFrame(Panel panel, Tuple<int, double> frame)
        {
            ...
            foreach (var con in panel.Controls.Cast<object>()
                                     .Where(con => con.GetType() == typeof(Chart)))
            {
                ...
                pointList.AddXY(index, frame.Item2);
        ...
        private void recordBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                RemoveRecording(activePanel);
            }
        }

        //--------------------------------------------------------------------------------
        /// <summary>
        /// Removes the recording from the table once a recording is finished
        /// </summary>
        /// <param name="panelFrame">Selected frame to remove from table</param>
        private void RemoveRecording(PanelFrame panelFrame)
        {
            if (HashTblRecord.Contains(panelFrame.BodyId))
            {
                HashTblRecord.Remove(panelFrame.BodyId);
            }
        }
        ...
        private void SaveRecordedValues(PanelFrame panelFrame)
        {
            if (HashTblRecord.Contains(panelFrame.BodyId))
            {
                ...
                {
                    sw.WriteLine("Start Time: ({0})\tEnd Time: ({1})\tFrame Count {2}",
                             panelFrame.StartTime, panelFrame.EndTime, frameSet.Count);
                    foreach (var points in frameSet)
                    {
                        sw.WriteLine("Frame {0}\tValue {1}", points.Item1, points.Item2);
                    }
                }
            }
        }
