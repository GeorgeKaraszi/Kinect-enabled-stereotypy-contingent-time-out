...
using System.Linq;
using WesternMichgian.SeniorDesign.KinectProject.GraphTools;
...
namespace WesternMichgian.SeniorDesign.KinectProject.Algorithms
{
    public class PeaksAndValleys
    {
        ...
        public int PeriodThreshold { get; }       = 3;

        //--------------------------------------------------------------------------------
        /// <summary>
        /// Calculates the period of the given wave dataset
        /// </summary>
        /// <param name="wave">Dataset of double values that make the wave</param>
                                   ...
                                   int clearReturnTheshold = 3)
        {
            ...
            return CalculateThePeriod(wave,pv, clearInconsistantPeriods, 
                clearReturnTheshold);
        }
        ...
        /// </returns>
        private List<Tuple<int, double>> FindPeaksAndValleys(List<double> wave)
        {
        ...
        private int CalculateThePeriod(List<double> wave, 
                                                      ...
                                                      List<double> wave)
        {
            ...
            int i = 0;
            bool pvpPattern = pv[0].Item2 > pv[1].Item2;

            //Check to see if the first index is a peak or valley, skip if it's peak.
            ...
            for (; i < pv.Count; i += 2)
            {
                ...
                else if (i + 1 < pv.Count)
                {
                    ...
                    var newIndex = (pvpPattern ? SMath.FindLargestValueIndex(subArray)
                                              : SMath.FindSmallestValueIndex(subArray))
                                              + pv[i + 1].Item1;
                                     ...
                                     List<double> wave)
        {
            ...
            bool vpvRoutine = wave[pvPair1.Item1] < wave[pvPair1.Item2];
            bool mlReached = false;
            ...
            do
            {
                ...
                {
                    case 0:
                        mlReached = vpvRoutine
                            ? SMath.IntervalRange(wave[index], wave[index + 1], stMidline)
                            : SMath.IntervalRange(wave[index + 1], wave[index], stMidline);
                    ...
                    default:
                        ...
                        {
                            mlReached = vpvRoutine
                                ? SMath.IntervalRange(wave[index], wave[index + 1],
                                                      enMidline)
                                : SMath.IntervalRange(wave[index + 1], wave[index],
                                                      enMidline);
                            if (mlReached)
                            {
                                passedMidline++;
