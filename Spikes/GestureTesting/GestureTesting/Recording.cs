...
namespace GestureTesting
{
    ...
    public class RecordingTable
    {
        ...
        /// </summary>
        public void Reset(String filename, int numframes)
        {
            foreach (String gesture in _hashTblRecord.Keys)
            {
                var classifier = (GestureInterpreter) _hashTblRecord[gesture];

                classifier.Reset(gesture, filename, BodyId, numframes);
            }
        }
    }
}
